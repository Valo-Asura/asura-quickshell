...
/**
 * Central state management for Asura Shell panels.
...
Singleton {
    ...
    property string hoverPanelName: ""
    property bool hoverTriggerHovered: false
    property bool hoverContentHovered: false
    property int hoverCloseDelayMs: 520
    ...
    Timer {
        ...
        repeat: false
        onTriggered: root.closeHoverPanelIfIdle()
    }
    ...
    property string settingsAboutView: "main" // "main" or "dependency"
    ...
    onNotificationCenterOpenChanged: {
        if (notificationCenterOpen) {
            ...
            dashboardOpen = false
            sessionOpen = false
        } else if (hoverPanelName === "notifications") {
            clearHoverPanelState()
        }
    }
    ...
    onQuickSettingsOpenChanged: {
        if (quickSettingsOpen) {
            ...
            dashboardOpen = false
            sessionOpen = false
        } else if (hoverPanelName === "quickSettings") {
            clearHoverPanelState()
        }
    }
    ...
    onQuickActionsOpenChanged: {
        if (quickActionsOpen) {
            ...
            dashboardOpen = false
            sessionOpen = false
        } else if (hoverPanelName === "quickActions") {
            clearHoverPanelState()
        }
    }
    ...
    onDashboardOpenChanged: {
        if (dashboardOpen) {
            ...
            spotlightOpen = false
            sessionOpen = false
        } else if (hoverPanelName === "dashboard") {
            clearHoverPanelState()
        }
    }
    ...
    function clearHoverPanelState() {
        hoverCloseTimer.stop()
        hoverPanelName = ""
        hoverTriggerHovered = false
        hoverContentHovered = false
    }

    function pinPanelOpen() {
        clearHoverPanelState()
    }

    function openPanelByHover(panelName, screen = null) {
        if (screen !== null) activeScreen = screen
        hoverPanelName = panelName
        hoverTriggerHovered = true
        hoverContentHovered = false
        hoverCloseTimer.stop()

        if (panelName === "notifications") {
            notificationCenterOpen = true
        } else if (panelName === "quickSettings") {
            quickSettingsOpen = true
        } else if (panelName === "dashboard") {
            dashboardOpen = true
        } else if (panelName === "quickActions") {
            quickActionsOpen = true
        }
    }

    function setHoverTrigger(panelName, hovered) {
        if (hoverPanelName !== panelName) return
        hoverTriggerHovered = hovered
        if (hovered) hoverCloseTimer.stop()
        else scheduleHoverClose(panelName)
    }

    function setHoverContent(panelName, hovered) {
        if (hoverPanelName !== panelName) return
        hoverContentHovered = hovered
        if (hovered) hoverCloseTimer.stop()
        else scheduleHoverClose(panelName)
    }

    function scheduleHoverClose(panelName) {
        if (hoverPanelName !== panelName) return
        if (hoverTriggerHovered || hoverContentHovered) {
            hoverCloseTimer.stop()
            return
        }
        hoverCloseTimer.interval = hoverCloseDelayMs
        hoverCloseTimer.restart()
    }

    function closeHoverPanelIfIdle() {
        if (hoverPanelName === "" || hoverTriggerHovered || hoverContentHovered) return

        const panelName = hoverPanelName
        clearHoverPanelState()
    ...
    function activateSettings() {
        const moveCmd = Hyprland.usingLua
            ? "hyprctl eval \"hl.dispatch(hl.dsp.window.move({ workspace = \\\"name:$CUR_WS\\\", window = \\\"address:$ADDR\\\", follow = false }))\""
        ...
        const cmd = `
            ...
            if [ -z "$ADDR" ] || [ "$ADDR" = "null" ]; then
                # Not open, so open it. Force a state edge in case Quickshell
                # still thinks the window exists after a compositor-side close.
