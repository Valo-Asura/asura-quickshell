...
Singleton {
    ...
    readonly property var smartActions: [
        ...
        { name: "HyprMod", subtitle: "Hyprland settings", id: "smart-hyprmod", icon: "tune", aliases: "hyprmod hyprland settings config gtk libadwaita wayland compositor", isPlugin: true, category: "Application", emoji: "", execute: () => { Quickshell.execDetached(["bash", "-lc", "if command -v hyprmod >/dev/null 2>&1; then exec hyprmod; else notify-send 'HyprMod is not installed' 'Run ~/.local/bin/asura-install-hypr-info-mod-once.sh'; fi"]); root.closeAll(); } },
