...
Singleton {
    ...
    readonly property var smartActions: [
        ...
        { name: "Hypr System Info", subtitle: "Hyprland diagnostics", id: "smart-hyprsysteminfo", icon: "info", aliases: "hypr system info hyprsysteminfo diagnostics version copy gpu compositor wayland", isPlugin: true, category: "Application", emoji: "", execute: () => { Quickshell.execDetached(["sh", "-c", "command -v hyprsysteminfo >/dev/null 2>&1 && exec hyprsysteminfo"]); root.closeAll(); } },
        { name: "HyprMod", subtitle: "Hyprland settings", id: "smart-hyprmod", icon: "tune", aliases: "hyprmod hyprland settings config gtk libadwaita wayland compositor", isPlugin: true, category: "Application", emoji: "", execute: () => { Quickshell.execDetached(["sh", "-c", "command -v hyprmod >/dev/null 2>&1 && exec hyprmod"]); root.closeAll(); } },
