...
Singleton {
    ...
    property bool hasRuntimeAssets: false
    property bool hasWorkshopWallpapers: false
    ...
    property string workshopPath: Directories.home.replace("file://", "") + "/.local/share/Steam/steamapps/workshop/content/431960"
    ...
    Timer {
        ...
        onTriggered: {
            const lowerCmd = HyprlandCompat.dspLower("class:.*wallpaperengine.*");
            Quickshell.execDetached(HyprlandCompat.isLua ? ["hyprctl", "eval", lowerCmd] : ["hyprctl", "dispatch", lowerCmd]);
            count++;
            if (count > 5) {
                count = 0;
                stop();
    ...
    function checkInitialApply() {
        if (Config.ready) {
            ...
            if (lastPath && lastPath !== "") {
                ...
                if (root.isInstalled && root.hasRuntimeAssets) root.applyInternal(lastPath);
            }
        }
    }
...
print(json.dumps(props))
    ...
    function apply(folderPath, previewPath = "") {
        if (!folderPath) return;
        if (!root.isInstalled || !root.hasRuntimeAssets) {
            root.errorMessage = root.statusMessage || "Wallpaper Engine runtime assets were not found";
    ...
    function applyInternal(path) {
        if (!root.isInstalled || !root.hasRuntimeAssets) return;
        root.resume();
    ...
    Timer {
        ...
        onTriggered: {
            ...
            if (root.assetsDir !== "") args.push("--assets-dir", root.assetsDir);
    ...
    function fetch() {
        ...
        errorMessage = "";
        if (!root.isInstalled) {
            root.errorMessage = "linux-wallpaperengine-git is installed incorrectly or missing from PATH";
            return;
        }
        if (!root.hasRuntimeAssets) {
            root.errorMessage = root.statusMessage || "Wallpaper Engine is not installed in Steam, so runtime assets are missing";
            return;
        }
...
print(json.dumps(wallpapers))
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    const data = JSON.parse(this.text);
                    root.hasWorkshopWallpapers = data.length > 0;
                    if (data.length === 0) root.errorMessage = "No Steam Workshop wallpapers found in " + root.workshopPath;
                    else { 
...
asset_suffixes = [
    ...
    "wallpaper_engine/share/assets",
    "Wallpaper Engine/share/assets",
]

command = shutil.which("linux-wallpaperengine")
assets_dir = ""
workshop_dir = ""
workshop_count = 0
...
for steamapps in steamapps_candidates:
    ...
    if not assets_dir:
        ...
        for suffix in asset_suffixes:
            path = os.path.join(common, suffix)
            if os.path.isdir(path):
                assets_dir = path
                break

    candidate_workshop = os.path.join(steamapps, "workshop/content/431960")
    if os.path.isdir(candidate_workshop):
        count = 0
        try:
            for name in os.listdir(candidate_workshop):
                if os.path.isfile(os.path.join(candidate_workshop, name, "project.json")):
                    count += 1
        except OSError:
            count = 0

        if count > workshop_count or not workshop_dir:
            workshop_dir = candidate_workshop
            workshop_count = count

status = ""
if not command:
    status = "linux-wallpaperengine-git is not installed"
elif not assets_dir:
    status = "Install Wallpaper Engine in Steam once; its runtime assets folder is missing"
elif workshop_count == 0:
    status = "Subscribe to Wallpaper Engine Workshop wallpapers in Steam, then reopen this page"
else:
    status = "Wallpaper Engine is ready"

print(json.dumps({
    "command": command or "",
    "assetsDir": assets_dir,
    "workshopPath": workshop_dir,
    ...
    "status": status
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    const data = JSON.parse(this.text);
                    root.isInstalled = data.command !== "";
                    root.hasRuntimeAssets = data.assetsDir !== "";
                    root.hasWorkshopWallpapers = data.workshopCount > 0;
                    root.assetsDir = data.assetsDir || "";
                    if (data.workshopPath && data.workshopPath !== "") root.workshopPath = data.workshopPath;
                    root.statusMessage = data.status || "";
                    if (!root.isInstalled || !root.hasRuntimeAssets || !root.hasWorkshopWallpapers) {
                        root.errorMessage = root.statusMessage;
                    } else {
                        root.errorMessage = "";
                        if (Config.ready && Config.options.appearance.background.liveWallpaperPath !== "" && !root.isRunning && !root.isApplying) {
                            root.checkInitialApply();
                        }
                    }
                } catch (e) {
                    root.isInstalled = false;
                    root.hasRuntimeAssets = false;
                    root.hasWorkshopWallpapers = false;
                    root.statusMessage = "Unable to inspect Wallpaper Engine installation";
                    root.errorMessage = root.statusMessage;
                }
            }
        }
